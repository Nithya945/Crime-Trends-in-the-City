...
tweet.qry <- function(from, to){
  out <- dbGetQuery(con, sprintf("SELECT username, date, retweets, favourites, tweet, id,ST_X(ST_Transform(geom,26971)), ST_Y(ST_Transform(geom,26971))  
                               FROM tweet_copy
                               WHERE date BETWEEN '%s' AND '%s'
                               AND geom && ST_MakeEnvelope(-88, 41.6, -87.5, 42.5, 4326);",from, to))
  return(out)
}
...
plot.spatial.kde = function(kde.theft.density, kde.theft.density.points)
{
  image.x.values = sort(unique(kde.theft.density.points[,1]))
  image.y.values = sort(unique(kde.theft.density.points[,2]))
  image(x = image.x.values, 
        y = image.y.values,
        z = matrix(kde.theft.density, ncol=length(image.y.values), byrow=FALSE),
